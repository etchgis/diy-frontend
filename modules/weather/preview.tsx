...
export default function WeatherPreview({
  ...
  // Fetch weather data in editor mode when slide is first shown or coordinates change
  useEffect(() => {
    if (isEditor && coordinates?.lat && coordinates?.lng) {
      fetchWeatherData(slideId);
    }
  }, [isEditor, slideId, coordinates?.lat, coordinates?.lng]);
