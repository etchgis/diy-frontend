...
import Template2Preview from "./preview"
...
export default function Template2Slide({ slideId, handleDelete, handlePreview, handlePublish, handleOpenSettings }: { slideId: string, handleDelete: (id: string) => void, handlePreview: () => void, handlePublish: () => void, handleOpenSettings: () => void }) {
  ...
  const showTitle = useTemplate2Store((state) => state.slides[slideId]?.showTitle !== false);
  const setShowTitle = useTemplate2Store((state) => state.setShowTitle);
  ...
  const saveStatus = useLocalSaveStatus(useTemplate2Store, slideId);
