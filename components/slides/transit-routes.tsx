...
export default function TransitRoutesSlide({
  ...
  const displayName = useTransitRouteStore(
    (state) => state.slides[slideId]?.displayName || ""
  );
  const setDisplayName = useTransitRouteStore((state) => state.setDisplayName);

  const errorMessage = useTransitRouteStore(
    (state) => state.slides[slideId]?.errorMessage || ""
  );
  const setErrorMessage = useTransitRouteStore(
    (state) => state.setErrorMessage
  );

  const setIsLoading = useTransitRouteStore((state) => state.setIsLoading);
  const isLoading = useTransitRouteStore(
    (state) => state.slides[slideId]?.isLoading || false
  );
        ...
        <div className="flex-1 bg-white min-w-0">
          <div className="p-6">
            ...
            <p className="text-[#606061] mb-6">
              Input the destinations that you would like for the map to show.
              The maximum number of routes that this map can accommodate is 6.
            </p>
            ...
            <div className="mb-6">
              ...
              {routes.length >= MAX_DESTINATIONS && (
                <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">
                  Maximum of {MAX_DESTINATIONS} destinations reached. Remove a
                  destination to add another.
                </div>
              )}
            ...
            {routes && routes.length > 0 && (
              <div className="mt-2">
                {routes.map((route: any, index: number) => (
                  <div
                    key={index}
                    className="flex mt-2 items-center justify-between bg-[#f4f4f4] p-2 rounded"
                  >
                    <span className="text-xs text-[#4a5568] truncate pr-2">
                      {route.name}
                    </span>
                    <Button
                      variant="ghost"
